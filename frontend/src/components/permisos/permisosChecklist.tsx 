...
import { Permission, CATEGORIAS_PERMISOS } from '@/types/permisos';

interface PermisosChecklistProps {
  permisos: Permission[];
  seleccionados: number[];  // IDs de permisos seleccionados
...
export function PermisosChecklist({ 
  ...
  // Verificar si todos los de una categoría están seleccionados
  ...
  const toggleCategoria = (categoriaId: string) => {
    ...
    permisosCat.forEach(p => {
      onChange(p.id, !todosSeleccionados);
    });
  };
